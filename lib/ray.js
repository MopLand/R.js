...
(function( win, doc ){
	...
	//是否支持WebP
	var ele = doc.createElement('canvas');
    if (!!(ele.getContext && ele.getContext('2d'))) {
        R.Browser.webp = ele.toDataURL('image/webp').indexOf('data:image/webp') == 0;
    }
	...
	*/
	...
	R.Datetime = function( date ){
		...
		var inti = function( date ){
			...
			//Unix时间戳格式
			if( date && R.Validate.Number( date ) ){
				this.self = new Date( parseInt(date) * 1000 );
				
			//其他时间格式
			}else if( date ){
				this.self = new Date( date );
		...
		inti.prototype = {
			...
			*/
			Serialize : function( encode, concat, oldval ){
				...
				for (var i=0; i<len; i++){
					...
					switch( ele.type ){
						...
						default:
							if( ele.type == 'password' && set.encrypt == 'md5' && CryptoJS ){
								val = CryptoJS.MD5( val ).toString();
							}
			...
			/*
				...
				val		值
			*/
			Collect : function( data, key, val, set ){
	...
	/*
		...
		参数：
			...
			config	配置
				...
				func		回调函数
	*/
	R.toast = function( style, html, config, func ){

		//配置选项
		var conf = config ? config : {};

		//显示时间
		var time = conf['time'] || 3;

		//分配ID
		var uniq = conf['unique'] || R.String().random( 10, 'R' );
		...
		//创建对象
		R( document.body ).append( "div" , { "id" : uniq, "className" : style, "innerHTML" : html } );
	...
	*/
	...
	/*
		...
		target		目标位置，默认为<head>
	*/
	R.assets = function( file, func, target ){
		...
		for( var i in file ){
			...
			var type = R.String( file[i] ).fileExt();
			var name, attr = {};
			...
			element = R( target ).append( name, attr ).change();
			object.push( element.item( 0 ) );
