...
(function( win, doc ){
	...
	*/
	...
	R.Form = function( form ){

		//内部类
		var inti = function( form ){
			this.self = form;
			return this;
		}
		...
		inti.prototype = {
			...
			*/
			Serialize : function( encode, concat, prefix ){
				...
				for (var i=0; i<len; i++){
					...
					var key	= encode(ele.name);
					var val	= ele.value;

					/*忽略没有名字元素*/
					if ( !key ) continue;

					var key = i == 0 ? key : prefix + key;
					...
					switch( ele.type ){
						...
						case "select-one":
							...
							data[key] = encode( opt.attributes && opt.attributes.value && !(opt.attributes.value.specified) ? opt.text : opt.value );
						break;

						case "select-multiple":
							for (var x = 0; x < ele.length; x++) {
								if( ele[x].selected ){
									data[key] = encode(ele[x].value);
								}
							}
						break;
